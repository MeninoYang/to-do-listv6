...
function restoreList(lista) {
    ...
    lista.tarefas.forEach(tarefa => {
        ...
        } else if (tarefa && tarefa.nome) {
            tarefaLi.textContent = tarefa.nome;
        } else {
            tarefaLi.textContent = 'Tarefa sem nome';
        }

        if (!tarefaLi.querySelector('.circle')) {
            const circle = document.createElement('span');
            circle.classList.add('circle');
            circle.onclick = function() {
                openImportanceModal(tarefa); 
            };
            tarefaLi.appendChild(circle);
        }

        if (!tarefaLi.querySelector('.delete-task')) {
            ...
            tarefaLi.appendChild(deleteButton);
        }
